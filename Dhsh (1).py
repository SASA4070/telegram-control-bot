...
def update_stats(user_id, approved):
    ...
    record_check_result(user_id, approved)
# ===================================================
...
def create_user(user_id, username, first_name, referrer_id=None):
    ...
    if user_id_str not in users_data:
        ...
        users_data[user_id_str] = {
            ...
            "approved_checks": 0,
            "banned": False,
            "plan": "VIP" if user_id == ADMIN_ID else "Free",
...
def check_points(user_id, required_points=1):
    """التحقق من النقاط وخصمها - الأدمن معفي"""
    user_id_str = str(user_id)
...
def add_points_cmd(message):
    """إضافة نقاط - /addpoints <user_id> <points> <سبب اختياري>"""
    if message.from_user.id != ADMIN_ID:
        return
    try:
        parts = message.text.split()
        if len(parts) < 3:
            bot.reply_to(message, "❌ الاستخدام: /addpoints <user_id> <points> <سبب اختياري>")
            return
        ...
        user_id_str = str(user_id)
        if user_id_str in users_data:
            users_data[user_id_str]["points"] = users_data[user_id_str].get("points", 0) + points
            save_users(users_data)
            bot.send_message(user_id, f"✅ تم إضافة {points} نقطة\n⭐ رصيدك: {users_data[user_id_str]['points']}\n📝 السبب: {reason}")
            bot.reply_to(message, f"✅ تم إضافة {points} نقطة للمستخدم {user_id}")
        else:
            bot.reply_to(message, f"❌ المستخدم {user_id} غير موجود")
    except ValueError:
        bot.reply_to(message, "❌ خطأ: تأكد من كتابة ID المستخدم وعدد النقاط بشكل صحيح")
    except Exception as e:
        bot.reply_to(message, f"❌ خطأ: {str(e)}")

@bot.message_handler(commands=['ban'])
def ban_cmd(message):
    """حظر مستخدم - /ban <user_id> <سبب اختياري>"""
    if message.from_user.id != ADMIN_ID:
        return
    try:
        parts = message.text.split(maxsplit=1)
        if len(parts) < 2:
            bot.reply_to(message, "❌ الاستخدام: /ban <user_id> <سبب اختياري>")
            return
        ...
        user_id_str = str(user_id)
        if user_id_str in users_data:
            users_data[user_id_str]["banned"] = True
            save_users(users_data)
        ...
        bot.reply_to(message, "❌ خطأ: تأكد من كتابة ID المستخدم بشكل صحيح")
    except Exception as e:
        bot.reply_to(message, f"❌ خطأ: {str(e)}")

@bot.message_handler(commands=['unban'])
def unban_cmd(message):
    """رفع الحظر - /unban <user_id>"""
    if message.from_user.id != ADMIN_ID:
        return
    try:
        parts = message.text.split()
        ...
        if user_id_str in users_data:
            users_data[user_id_str]["banned"] = False
            save_users(users_data)
            bot.send_message(user_id, "✅ تم رفع الحظر عن حسابك\nيمكنك استخدام البوت مرة أخرى")
...
def users_cmd(message):
    ...
    for uid, info in list(users.items())[:20]:
        name = info.get('name', 'غير معروف')[:10]
        points = info.get('points', 0)
        banned = "🚫" if info.get('banned', False) else "✅"
        plan = info.get('plan', 'Free')
        msg += f"• `{uid}` {banned} | {name} | {plan} | ⭐{points}\n"
...
def userinfo_cmd(message):
    """معلومات مستخدم - /userinfo <user_id>"""
    if message.from_user.id != ADMIN_ID:
        return
    try:
        parts = message.text.split()
        if len(parts) < 2:
            bot.reply_to(message, "❌ الاستخدام: /userinfo <user_id>")
            return
        ...
        if user_id_str in users_data:
            info = users_data[user_id_str]
            
            success_rate = 0
            total_checks = info.get('total_checks', 0)
            approved_checks = info.get('approved_checks', 0)
            if total_checks > 0:
                success_rate = (approved_checks / total_checks) * 100
...
📋 **الخطة:** {info.get('plan', 'Free')}
🚫 **محظور:** {'نعم ❌' if info.get('banned', False) else 'لا ✅'}
━━━━━━━━━━━━━━━━
📊 **إجمالي الفحوصات:** {total_checks}
✅ **الفحوصات المقبولة:** {approved_checks}
❌ **الفحوصات المرفوضة:** {total_checks - approved_checks}
📈 **نسبة النجاح:** {success_rate:.1f}%
━━━━━━━━━━━━━━━━
📅 **تاريخ التسجيل:** {info.get('join_date', 'غير معروف')}
"""
...
def stats_users_cmd(message):
    """إحصائيات المستخدمين - /stats_users"""
    if message.from_user.id != ADMIN_ID:
        return
...
━━━━━━━━━━━━━━━━
💳 **إجمالي الفحوصات:** {total_checks}
✅ **إجمالي المقبولة:** {total_approved}
📈 **نسبة النجاح:** {(total_approved/total_checks*100) if total_checks > 0 else 0:.1f}%
━━━━━━━━━━━━━━━━
"""
    bot.reply_to(message, msg)

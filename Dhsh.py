...
from user_manager import (
    get_user_plan, set_user_plan, add_points, get_points, 
    ban_user, unban_user, get_all_users, get_user_info,
    check_user_status, is_vip, deduct_points
)

@bot.message_handler(commands=['set_plan'])
def set_plan_command(message):
    """/set_plan <user_id> <plan> [expiry_date]"""
    if message.from_user.id != ADMIN_ID:
        return
    ...
    try:
        ...
        if len(parts) < 3:
            bot.reply_to(message, "❌ الاستخدام: /set_plan <user_id> <plan>\nالخطط المتاحة:\n• Free - Not Subscribed\n• VIP - شهر\n• VIP - سنة\n• VIP - مدى الحياة")
            return
        ...
        if "شهر" in plan:
            ...
            from datetime import timedelta
            expiry_date = (datetime.now() + timedelta(days=365)).strftime("%Y-%m-%d %H:%M")
        
        set_user_plan(user_id, plan, expiry_date)
        bot.reply_to(message, f"✅ تم تعيين خطة {plan} للمستخدم {user_id}")
        
        # إشعار للمستخدم
        bot.send_message(user_id, f"🎉 تم ترقية حسابك إلى {plan}")
        if expiry_date:
            bot.send_message(user_id, f"📅 صلاحية الاشتراك حتى: {expiry_date}")
...
def add_points_admin(message):
    ...
    try:
        parts = message.text.split(maxsplit=2)
        if len(parts) < 3:
            bot.reply_to(message, "❌ الاستخدام: /add_points <user_id> <points> <سبب اختياري>")
            return
        
        user_id = int(parts[1])
        points = int(parts[2])
        reason = " ".join(parts[3:]) if len(parts) > 3 else "إضافة يدوية"
        
        new_points = add_points(user_id, points, reason)
...
def deduct_points_admin(message):
    """/deduct_points <user_id> <points> [reason]"""
    if message.from_user.id != ADMIN_ID:
        return
    ...
    try:
        parts = message.text.split(maxsplit=2)
        if len(parts) < 3:
            bot.reply_to(message, "❌ الاستخدام: /deduct_points <user_id> <points> <سبب اختياري>")
            return
        
        user_id = int(parts[1])
        points = int(parts[2])
        reason = " ".join(parts[3:]) if len(parts) > 3 else "خصم يدوي"
        
        if deduct_points(user_id, points, reason):
            bot.reply_to(message, f"✅ تم خصم {points} نقطة من المستخدم {user_id}")
            bot.send_message(user_id, f"❌ تم خصم {points} نقطة من رصيدك\n📝 السبب: {reason}")
        else:
            bot.reply_to(message, f"❌ رصيد المستخدم {user_id} غير كافي")
...
def ban_user_command(message):
    ...
    try:
        parts = message.text.split(maxsplit=1)
        if len(parts) < 2:
            bot.reply_to(message, "❌ الاستخدام: /ban_user <user_id> <سبب اختياري>")
            return
        
        user_id = int(parts[1].split()[0])
        reason = " ".join(parts[1].split()[1:]) if len(parts[1].split()) > 1 else "لا يوجد سبب"
        
        ban_user(user_id, reason)
        bot.reply_to(message, f"✅ تم حظر المستخدم {user_id}\n📝 السبب: {reason}")
        
        # إشعار للمستخدم
        bot.send_message(user_id, f"🚫 تم حظر حسابك من البوت\n📝 السبب: {reason}\n📞 للاستفسار: @s3s_a")
...
def unban_user_command(message):
    ...
    try:
        parts = message.text.split()
        if len(parts) < 2:
            bot.reply_to(message, "❌ الاستخدام: /unban_user <user_id>")
            return
        ...
        if unban_user(user_id):
            ...
            bot.send_message(user_id, f"✅ تم رفع الحظر عن حسابك\nيمكنك استخدام البوت مرة أخرى")
        else:
...
def user_info_command(message):
    ...
    try:
        parts = message.text.split()
        if len(parts) < 2:
            bot.reply_to(message, "❌ الاستخدام: /user_info <user_id>")
            return
...
━━━━━━━━━━━━━━━━
📋 **الخطة:** {info.get('plan', 'Free - Not Subscribed')}
⭐ **النقاط:** {info.get('points', 0)}
🚫 **محظور:** {'نعم' if info.get('banned', False) else 'لا'}
📅 **تاريخ التسجيل:** {info.get('joined', 'غير معروف')}
"""
            if info.get('expiry'):
                msg += f"📅 **صلاحية VIP:** {info.get('expiry')}\n"
            if info.get('ban_reason'):
                msg += f"📝 **سبب الحظر:** {info.get('ban_reason')}\n"
...
def all_users_command(message):
    ...
    for uid, info in list(users.items())[:20]:  # عرض أول 20 مستخدم
        plan = info.get('plan', 'Free')
        points = info.get('points', 0)
        banned = "🚫" if info.get('banned', False) else "✅"
        msg += f"• `{uid}` {banned} | {plan} | ⭐{points}\n"
    
    msg += f"\n📊 إجمالي المستخدمين: {len(users)}"
    bot.reply_to(message, msg, parse_mode="Markdown")

@bot.message_handler(commands=['stats_users'])
def stats_users_command(message):
    """/stats_users - إحصائيات المستخدمين"""
    if message.from_user.id != ADMIN_ID:
        return
    ...
    total = len(users)
    vip = sum(1 for u in users.values() if "VIP" in u.get("plan", ""))

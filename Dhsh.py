...
def format_approved_result(gateway_name, card, response, exec_time):
    """تنسيق نتيجة النجاح بالشكل المطلوب"""
...
def format_declined_result(gateway_name, card, response, exec_time):
    """تنسيق نتيجة الرفض بالشكل المطلوب"""
    bin_info = get_bin_info(card[:6])
...
def check_single_card(message, card, gateway_func, gateway_name):
    ...
    try:
        ...
        update_stats(user_id, "Approved" in result or "✅" in result)
...
📌 <b>ملاحظة:</b> استبدل xxxxxxxx بأرقام بطاقتك
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
...
def gen_command(message):
    ...
    if cards:
        msg = f"🎴 <b>تم توليد 10 بطاقة من BIN {bin_input[:6]}</b>\n\n" + "\n".join([f"<code>{c}</code>" for c in cards])
...
def handle_callbacks(call):
    ...
    if data == "back_main":
        ...
        # الآن get_check_menu ترجع نص وليس أزرار
        ...
        try:
            bot.edit_message_text("🎲 <b>اختر طريقة التوليد</b>", 
                                  call.message.chat.id, call.message.message_id, 
                                  reply_markup=get_gen_menu())
        except:
            bot.send_message(call.message.chat.id, "🎲 <b>اختر طريقة التوليد</b>", 
                             reply_markup=get_gen_menu())
        bot.answer_callback_query(call.id)
        return
        ...
        try:
            bot.edit_message_text("🔍 أرسل الـ BIN\nمثال: <code>/bin 528445</code>\nأو أرسل الـ BIN مباشرة", 
                                  call.message.chat.id, call.message.message_id, parse_mode='HTML')
        except:
            bot.send_message(call.message.chat.id, "🔍 أرسل الـ BIN\nمثال: <code>/bin 528445</code>\nأو أرسل الـ BIN مباشرة", parse_mode='HTML')
        bot.answer_callback_query(call.id)
        return
    
    elif data == "fake_menu":
        try:
            bot.edit_message_text("🌍 <b>اختر الدولة</b>", 
                                  call.message.chat.id, call.message.message_id, 
                                  reply_markup=get_fake_country_menu())
        except:
            bot.send_message(call.message.chat.id, "🌍 <b>اختر الدولة</b>", 
                             reply_markup=get_fake_country_menu())
        bot.answer_callback_query(call.id)
        return
...
Dev ➜ @s3s_a"""
    ...
    # أزرار توليد البطاقات الجديدة
        ...
        try:
            bot.edit_message_text("📚 أرسل البينات مفصولة بفواصل أو أسطر\nمثال: 528445, 518928, 410621\nأو كل بين في سطر", 
                                  call.message.chat.id, call.message.message_id)
        except:
            bot.send_message(call.message.chat.id, "📚 أرسل البينات مفصولة بفواصل أو أسطر\nمثال: 528445, 518928, 410621\nأو كل بين في سطر")

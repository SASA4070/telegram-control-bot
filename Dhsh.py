...
# حط الجديد هنا تحت مباشرة 👇

def gateway_action_menu(command_name):

    markup = types.InlineKeyboardMarkup(row_width=1)

    markup.add(
        types.InlineKeyboardButton(
            f"💳 Single Check ➜ {command_name}",
...
def check_single_card_msg(message, check_func, gateway_name):
    ...
    status_msg = bot.reply_to(message, "⏳ Checking Card...")
    ...
    try:
        ...
        safe_result = html.escape(result[:120])

        bin_info = get_bin_info(card[:6])

        safe_bin = html.escape(bin_info)
...
def start_mass_check(call, user_id, cards, check_func, gateway_name):
    ...
    def build_mass_keyboard(status_text):
...
<b>Gateway ➜</b> <code>{gateway_name}</code>
...
"""
    ...
    msg = bot.send_message(
        ...
        start_text,
        parse_mode="HTML",
        reply_markup=build_mass_keyboard("⏳ Processing")
    )
...
"""
            ...
            try:
                ...
                safe_result = html.escape(result[:60])
                ...
                if is_live:
                    ...
                    short_status = "DECLINED"

                results_text += f"""
{status_icon} <code>{card}</code>
└ <b>{short_status}</b> ➜ <code>{safe_result}</code>

"""

                updated_text = f"""
<b>MASS CHECK RUNNING ⚡</b>
...
"""
        ...
        try:
            ...
            bot.edit_message_text(
                ...
                msg.message_id,
                parse_mode="HTML",
                reply_markup=build_mass_keyboard("✅ Completed")
            )
...
⭐ رصيدك: {users_data.get(str(user_id), {}).get('points', 10)} نقطة
"""
...
def handle_callbacks(call):
    ...
    # ================= قوائم الفحص =================
        ...
        safe_edit(
            ...
            call.message.chat.id,
            call.message.message_id,
            reply_markup=braintree_auth_menu()
        )
    ...
    elif data == "gateway_st1":

        pending_gateway[user_id] = (Stripe1, "Stripe1")

        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Stripe1"
        )

    elif data == "gateway_st2":

        pending_gateway[user_id] = (Stripe2, "Stripe2")

        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Stripe2"
        )

    elif data == "gateway_st3":

        pending_gateway[user_id] = (Stripe3, "Stripe3")

        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Stripe3"
        )

    elif data == "gateway_st4":

        pending_gateway[user_id] = (Stripe4, "Stripe4")

        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Stripe4"
        )

    elif data == "gateway_st5":

        pending_gateway[user_id] = (Stripe5, "Stripe5")

        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Stripe5"
        )

    elif data == "gateway_st6":

        pending_gateway[user_id] = (Stripe6, "Stripe6")

        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Stripe6"
        )
    ...
    # ================= Braintree =================
        ...
        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Braintree1"
        )
        ...
        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Braintree2"
        )
    ...
    # ================= Charge =================
        ...
        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Donate"
        )
        ...
        bot.send_message(
            call.message.chat.id,
            "💳 أرسل البطاقة لفحصها عبر Braintree Charge"
        )
    ...
    # ================= الملفات =================
        ...
        safe_edit(
            ...
            call.message.chat.id,
            call.message.message_id,
            reply_markup=file_braintree_list()
        )
    ...
    # ================= معلومات =================
        ...
        safe_edit(
            "📊 رد على رسالة فيها بطاقات ثم ارسل /mass",
            call.message.chat.id,
            call.message.message_id
        )
        ...
        safe_edit(
            f"📊 الفحوصات: {user.get('total_checks',0)}\n✅ المقبولة: {user.get('approved_checks',0)}",
            call.message.chat.id,
            call.message.message_id
        )
